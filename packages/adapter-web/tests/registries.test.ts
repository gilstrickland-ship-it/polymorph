...
describe("retrofit shim", () => {
  it("flattens to a pm.* → concrete value map (NOT var() refs)", () => {
    const map = toTokenMap(makeResolved("light"));
    const keys = Object.keys(map);
    expect(keys.every((k) => k.startsWith("pm."))).toBe(true);
    // concrete values, not CSS strings — the existing-SDK shim target
    expect(typeof map["pm.color.surface.base"]).toBe("string");
    expect(map["pm.radius.control"]).toHaveProperty("value");
  });
});
