...
- **70 tokens** total — **42 required**, 28 optional.
...
| `pm.color.action.{primary,secondary,danger}.*` | color | `primary` rest / pressed / disabled (req) + hover; `secondary` / `danger` rest (req) + pressed |
...
| `pm.motion.duration.*` / `pm.motion.easing.*` | duration / cubicBezier | durations short / base / reduced (req) + long; easings standard (req) + emphasized / reduced |
...
A minimal valid `light` theme must supply all 42 required tokens. Generate the current checklist
from the manifest:
