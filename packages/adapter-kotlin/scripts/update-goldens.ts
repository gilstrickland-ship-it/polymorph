/**
 * Regenerate every committed Kotlin golden under `tests/golden/`. Run after intentionally
...
import { transformToKotlin } from "../src/index.js";
...
for (const bank of banks) {
  for (const mode of modes) {
    ...
    const kt = transformToKotlin(bank.theme, { mode, objectName });
    const path = join(goldenDir, `${bank.name}_${mode}.kt`);
    writeFileSync(path, kt);
    count++;
    console.log(`wrote ${bank.name}_${mode}.kt (${kt.length} bytes, object ${objectName})`);
  }
}
