/**
 * Regenerate every committed Swift golden under `tests/golden/`. Run after intentionally
...
import { transformToSwift } from "../src/index.js";
...
for (const bank of banks) {
  for (const mode of modes) {
    ...
    const swift = transformToSwift(bank.theme, { mode, enumName });
    const path = join(goldenDir, `${bank.name}_${mode}.swift`);
    writeFileSync(path, swift);
    count++;
    console.log(`wrote ${bank.name}_${mode}.swift (${swift.length} bytes, enum ${enumName})`);
  }
}
